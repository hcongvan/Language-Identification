...
tf.flags.DEFINE_string("dataset", "./data", "Dataset name.")
tf.flags.DEFINE_string("output_dir", "./output", "Optional output dir.")
...
def experiment_fn(run_config, hparams):
    ...
    estimator = tf.contrib.learn.Estimator(
        model_fn=make_model_fn(), config=run_config, params=hparams)#fix
    return tf.contrib.learn.Experiment(
        estimator=estimator,
        train_input_fn=make_input_fn(tf.estimator.ModeKeys.TRAIN, hparams), #fix
        eval_input_fn=make_input_fn(tf.estimator.ModeKeys.EVAL, hparams),#fix
        eval_metrics=MODELS[FLAGS.model].eval_metrics(hparams),#fix
...
def main(unused_argv):
    ...
    DATASETS[FLAGS.dataset].prepare()#fix
